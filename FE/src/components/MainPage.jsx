...
export default function MainPage ({token}) {
    ...
    useEffect(() => {
        ...
        let randRecipeState = selectRecipe(dummyRecipes);
    ...
    return (
        ...
        <h2>{featRecipe.title}</h2>
                       <h4>{featRecipe.userName}</h4>
                       <img src={featRecipe.imgUrl} alt={`A picture of ${featRecipe.title}`} height="15%" width="22.5%"/>
                       <p><em>{featRecipe.tags}</em></p>
                       <p>Est. Time: {featRecipe.estimatedTime}</p>
                       <button onClick={() => {
                           navigate(`/recipes/${featRecipe.id}`)
                       }}>See Recipe</button>
        </div>
